  ...
  )
{
  ...
  if (FaultReg & (B_FSTS_REG_IQE | B_FSTS_REG_ITE | B_FSTS_REG_ICE)) {
    ...
    MmioWrite32(mVtdUnitInformation[VtdIndex].VtdUnitBaseAddress + R_FSTS_REG, 0xFFFFFFFF);
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (mVtdUnitInformation[VtdIndex].EnableQueuedInvalidation == 0) {
      ...
      // Removed error check for ICC bit
      // Proceed directly to setting command
...
)
{
    if (mVtdUnitInformation[VtdIndex].EnableQueuedInvalidation == 0) {
        ...
        Reg64 = MmioRead64(mVtdUnitInformation[VtdIndex].VtdUnitBaseAddress +
            (mVtdUnitInformation[VtdIndex].ECapReg.Bits.IRO * 16) +
            R_IOTLB_REG);
        ...
        // ÉèÖÃÃèÊö·û
        QiDesc.Uint64[0] = QI_IOTLB_DID(0) |
            ...
            QI_IOTLB_GRAN(1) |
            QI_IOTLB_TYPE;
        QiDesc.Uint64[1] = QI_IOTLB_ADDR(0) |
            QI_IOTLB_IH(0) |
            QI_IOTLB_AM(0);
        QiDesc.Uint64[2] = 0;
        QiDesc.Uint64[3] = 0;
